...
TEST_CASE("Testing enqueue edge cases"){
	...
	SECTION("Adding an item with higher priority (earlier in time) to a priority queue with exactly one item"){
        ...
        StartCPU* laterEvent = new StartCPU(2, earliestProcess, sim);
		
		testQ->enqueue(laterEvent);

		ProcessArrival * earlierEvent = dynamic_cast<ProcessArrival * >(testQ->getFront());
		
		if(earlierEvent){
			REQUIRE(earlierEvent->compareTo(testQ->getFront()) == 0);
		}
